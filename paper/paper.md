...
BLiSS, the Blind Line Search System, is a modular Python package for the automated detection and optional identification of emission-line candidates in astronomical spectra. The package was developed with X-ray spectroscopy in mind, but its core algorithms operate on one-dimensional spectra represented by energy, flux or counts, and uncertainty arrays. BLiSS is designed for exploratory spectral analysis, where the objective is to identify candidate emission structures before committing to a detailed physical continuum model.
...
BLiSS addresses this need by providing a blind, continuum-independent candidate-detection workflow. Instead of requiring a predefined global continuum model or emission line list, BLiSS estimates empirical local baselines directly from the input spectrum, detects positive excess structures, characterizes them with Gaussian models, and evaluates their reliability using synthetic comparison populations and Gaussian Mixture Models (GMM). Optional atomic-line identification routines can then match fitted centroids against transition tables within user-defined Doppler windows.

# Statement of need

Several mature tools already exist for astronomical spectral analysis. XSPEC [@1996ASPC..101...17A], SPEX [@kaastra1996uv], ISIS [@2013ascl.soft02002H], jaxspec[2024A&A...690A.317D], bxa[2021JOSS....6.3045B] and Sherpa [@2001SPIE.4477...76F] provide powerful environments for instrument-aware fitting and physical plasma modelling. These packages are essential for final quantitative interpretation. However, they are primarily designed around explicit spectral models and user-specified fitting components, rather than blind candidate discovery.
...
BLiSS combines real and synthetic candidates in a multidimensional feature space, using quantities such as Gaussian amplitude, width, signal-to-noise ratio, local contrast, and width-to-amplitude ratio. These features are standardized before GMM are fitted. The number of GMM components is selected using the Bayesian Information Criterion (BIC). For each cluster, BLiSS estimates the probability of their compoents and evaluate wheather they are real emission-line candidates or artifacts, based on the relative number of real and synthetic features assigned to that cluster.
...
An integration with ISIS is also provided: it directly adds Gaussian emission lines to a chosen continuum.
